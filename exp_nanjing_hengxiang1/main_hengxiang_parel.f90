!***************************************************
    ...
    REAL :: coef
    real *8,allocatable:: bindarr(:,:)

    real *8,allocatable:: rho_iter(:), hh_iter(:), merge_para(:), merge_dobs(:)
    ...
    open (33, File='time_record.txt', status='unknown')
    ...
    tot_ntc = ntc*ns      ! 1904
    size0 = (ns-1)*(nolayer-1)  ! 198
    size1 = (ns-1)*npara ! 429

    eps = 1.d-10

!    size2 = tot_ntc                ! 无约束
    size2 = tot_ntc+size1        ! 横向约束
    !  size2 = tot_ntc+size1+size0  ! 横向和纵向约束

    ka=max(size2,tot_para)+1

    allocate(counts(ns), displs(ns))
    allocate(bindarr(ns-1, npara))
    ...
    allocate(m_pre(tot_para,1),m_next(tot_para,1),Vobs(tot_ntc,1),Vobs_tot(ns*ntc,1))
    ...
    allocate(rho_iter(nolayer), hh_iter(nolayer), merge_para(npara), merge_dobs(ntc))

    allocate(rho_pro(nolayer*ns), hh_pro(nolayer*ns))
    ...
    Open (7, File='log.dat', Status='unknown')
    Open (8, File='err.dat', Status='unknown')
    Open (81, File='Wp.dat', Status='unknown')
    ...
    Vobs = Vobs_tot  ! 不用转换 ! /1000.0d0   ! 将mV转换为V
...
101 Continue
  call SYSTEM_CLOCK(iTimes2)
    ...
    if(counter .eq. 1)then
        do i=1,nolayer
            rho_iter(i) = rho_pro(i+(k-1)*nolayer)
            if(i .lt. nolayer)then
                hh_iter(i) = hh_pro(i+(k-1)*nolayer)
            end if
        end do
    ...
    do i=1,npara
        if(i .le. nolayer)then
            merge_para(i) = dlog(rho_iter(i))
        else
            merge_para(i) = dlog(hh_iter(i-nolayer))
        end if
    end do


    call MPI_AllGather(merge_para, npara, MPI_DOUBLE_PRECISION, &
                       m_pre, npara, MPI_DOUBLE_PRECISION, &
                       MPI_COMM_WORLD, ierr )

    ! 计算当前测点的误差
    do i=1,ntc
        merge_dobs(i) = Vobs(i+(k-1)*ntc,1)-hz1_iter(i)
    end do
    
    call MPI_AllGather(merge_dobs, ntc, MPI_DOUBLE_PRECISION, &
    deltadobs, ntc, MPI_DOUBLE_PRECISION, &
                        MPI_COMM_WORLD, ierr )
    ...
    ! 总的雅可比矩阵
    ! ********** Define MPI datatype for Jacobi matrix ***************
    sizes    = [ ns*ntc, ns*npara ]
    subsizes = [ ntc, npara ]
    starts   = [ 0, 0 ]
    ...
    call MPI_AllGatherv( jacobi1, ntc*npara, MPI_DOUBLE_PRECISION, & ! 
                      jacobi, counts, displs, resizedtype,&
                       MPI_COMM_WORLD, ierr)
    ...
    deltaRp = matmul(-Rp,m_pre)
    ! write(7,*)'deltaRp:'
    ! write(7,10)deltaRp


    ! Write(7,*)'calculate matrix A ...'
    do i=1,size2
    ...
    temp0 = matmul(transpose(deltaRp),deltaRp)
    rerror2= temp0(1,1)/size1

    temp0 = matmul(transpose(b0),b0)
    rerror3= temp0(1,1)/(tot_ntc+size1)
  ...
  call SYSTEM_CLOCK(iTimes3)
    ...
    if(rank==0)then
        ...
        write(33,*) 'Iteration ',counter,' time cost: ', real(iTimes3-iTimes2)/real(rate)!80/1000!
        write(*,*) 'Iteration ',counter,' time cost: ', real(iTimes3-iTimes2)/real(rate)!80/1000!

    end if


    If(rerror>epsi .and. counter<31) Goto 101
...
360 ks = ks - 1
    ...
    integer:: i,j,no,ntc0,ns_id1,ns_real1
    real *8 htt1
    ...
    integer:: i,k,jth,ntc1, ns_id,ns_real2
